...
class RideInfoController extends GetxController {
  ...
  getLatestRide() async {
    ...
    (await _getRidesUseCase.getLatestRide()).fold(
        (failure) => stateController.add(ErrorState(
  ...
  deletRide() async {
    stateController.add(LoadingState(
        stateRendererType: StateRendererType.fullScreenLoadingState));
    (await _getRidesUseCase.deletRide(ride!.id.toString())).fold(
        (failure) => stateController
